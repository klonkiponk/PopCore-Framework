...
function con_ListFolder($path)
{
    ...
    $imagesArray = array('jpeg','png','jpg','gif','png','JPG','JPEG','ico','svg','GIF');
    ...
    echo "<div class='imageBrowserFolder'><h1>$dirname</h1>\n";
    echo "<ul class='listFolderUl' style='display:block;'>\n";
    while (false !== ($file = readdir($dir_handle))) {
        if($file!="." && $file!="..") {
            if (is_dir($path."/".$file)) {
                //list of subFolders.
                con_ListFolder($path."/".$file);
            } else {

                $fileArray = explode(".", $file);
	                
                if (in_array($fileArray[1],$imagesArray)) {
	                echo "
	                <div class='imageBrowser' style='width:170px; height:300px;display:inline;float:left;margin:5px;'>
	                <img style=\"width:150px;height:200px;margin 5px;\" class='floatCenter' src=\"$path/$file\"/>
	                <br/>
	                <form method='post'>
	                <input type='text' style='margin:0px auto;display:block; width:150px;' name='img' value='$path/$file'/>
	                
	                <hr class='clear'><button style='margin:0px; display:block; margin:0px auto;' type='submit' class='button delete' name='action' value='deleteImage'>löschen</button></form>
	                </div>";
                } else {  //List Of Files                
                	//echo "<li class='listFolderLi'><a href='$path/$file'>$file</a></li>";
                }
            }
        }
    }
